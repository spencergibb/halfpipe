...
public class Generics {
    ...
    public static <T> Class<T> getTypeParameter(Class<?> klass, Class<? super T> bound) {
        ...
        if (t == null) { // lets try interfaces rather than classes
            Type[] genericInterfaces = klass.getGenericInterfaces();
            for (Type iface: genericInterfaces) {
                if (iface instanceof ParameterizedType) {
                    Class<T> cls = findClass((ParameterizedType) iface, bound);
        ...
        if (t instanceof ParameterizedType) {
            Class<T> cls = findClass((ParameterizedType) t, bound);
            if (cls != null) { return cls; }
        }
        throw new IllegalStateException("Cannot figure out type parameterization for " + klass.getName());
    }

    private static <T> Class<T> findClass(ParameterizedType t, Class<? super T> bound) {
        // should typically have one of type parameters (first one) that matches:
        for (Type param : t.getActualTypeArguments()) {
